...
begin
	pmtdir  ="/Users/jj/JuliaProjects/LaserLab/labdata/PMT"
	file    ="C1--Trace--00010.trc"
	fw      ="C1--Trace--00108.trc"
	path = joinpath(pmtdir,fw)
	io = open(path, "r")
	#close(io)
end
...
# ╔═╡ 581f1224-6393-4455-a809-9470b022bb6b
md"""
## Readout functions
"""

# ╔═╡ 0503357f-f671-4e8a-853b-49641e6f75d2
""" Returns the adress of the reference mark in the stream"""
function wavedesc(io)
	seek(io, 0)
	header = Vector{UInt8}(undef, 20)
	readbytes!(io, header)
	header
	sh = String(header)
	result = findfirst("WAVEDESC", sh)
	WAVEDESC = first(result) -1 
end

# ╔═╡ d93df476-7083-4c7c-826f-0327fb52fb6b
WAVEDESC = wavedesc(io)

# ╔═╡ 9a0d7e96-9a5b-4d04-aa5c-5ea72ec577ce
""" reads two bytes at location s and unpack them as an int16"""
function readword(s, pos)
	seek(s, pos)
...
# ╔═╡ 83df4bc4-20df-4619-85c3-11da7a3c6896
""" reads four bytes at location s and unpack them as an Int32"""
function readlong(s, pos)
	seek(s, pos)
...
# ╔═╡ f254857b-35b1-486c-890d-fcbde06218c4
""" reads four bytes at location s and unpack them as an Float32"""
function readfloat(s, pos)
	seek(s, pos)
	bytes = Vector{UInt8}(undef, 4)
	readbytes!(s, bytes)
...
# ╔═╡ 534e15eb-97cf-4919-8a50-f47037c20097
...
# ╔═╡ 8d030f1f-169f-4e7c-aadf-c2a3486e1ac6
""" reads eight bytes at location s and unpack them as an Float64"""
function readstring(s, pos, strlen)
	seek(s, pos)
...
# ╔═╡ 12f44771-acd5-45a2-bf1e-a4c3511efaed
ceil(5.4)

# ╔═╡ 5fe29906-e979-485f-bd24-066a88ee4791
""" reads the time stamp"""
function readtimestamp(s, pos)
	seek(s,pos)
	bytes = Vector{UInt8}(undef, 8)
	readbytes!(s, bytes)
...
# ╔═╡ 3477ddc5-d323-47b4-b9b3-6c0cde083f55
address = Dict("WAVEDESC"             => WAVEDESC,
...
# ╔═╡ 5bc7742c-a8ef-44b0-b053-0f925fc6a3c7
""" reads the waveform in y"""
function read_ywave(s, addr)
...
# ╔═╡ 8ed0ce0d-8bc9-4db2-b9ee-0afa45c9e38f
""" Returns the (time, ampl) data (t in mus ampl in mV)"""
function xydata(io, addr)
...
# ╔═╡ cfbc6c2a-e1f2-46f1-b64b-664e30a686a7
wvfm = xydata(io, address)
...
# ╔═╡ d268e933-65b9-4317-8fa0-054130b5b0de
"""
...
# ╔═╡ b9ec3dec-93fa-4db5-bcec-8129fd4f1759
stats = wstats(wvfm; nsigma=3)
...
# ╔═╡ 732ec309-d322-407f-b21c-0a1b8485f8c6
function plot_waveform(wvfm, stats; sct=false, trace=false)
	if sct
...
# ╔═╡ 8e6949c9-3656-4471-a9ff-0d046580fa1c
plot_waveform(wvfm, stats; sct=false, trace=false)
