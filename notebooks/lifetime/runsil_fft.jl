...
# ╔═╡ 6fa25494-93a2-48ad-892e-5081d0914400
md"## Characterising signals"

# ╔═╡ f88b142f-c42b-46d7-90e1-a657946df80e
md"### Plotting waveform"

# ╔═╡ 43f951f5-3c82-4c9a-8e22-249310dfa588
md"### Plotting waveform FFT"

# ╔═╡ 7f638d01-4f8e-46ce-bc98-6eb73763e213
md"- Notice that noise frequency appear above 200 MHz, thus a low-pass filter can be used to filter them away"

# ╔═╡ ef104366-3c34-4658-9317-25e8c438cdcc
md"## Filtering the signal"

# ╔═╡ 4e35809a-0a39-432e-8175-bb06fb571194
md"### Butterworth Filter"

# ╔═╡ 4ba8b710-3760-4647-8294-5c5f0be7d97b
md"### Chebyshev filter"

# ╔═╡ 84dba0b8-fe30-42ee-8542-233160cb4bf6
md"### Same features of the filters"

# ╔═╡ 35436842-9dc0-4541-8d45-abf3d174ccd1
md" ## Filtering signals using a Chebyshev filter
"
...
# ╔═╡ 5a03e28b-921a-4f33-a8f2-2e642b2f8f01
md"### Subtract the base line"

# ╔═╡ 3e54ffba-6521-4236-bafd-0caaa207a518
md"### Find the maxima in the landscape"

# ╔═╡ f8c63ce1-9365-4de0-aa3b-dbc04d82aeb1
promsel=0.5

# ╔═╡ 7ebf77fd-cd26-421a-82d2-7cbb74e3c4e7
md"### Find peak prominence
- minimum peak prominence to retain the peak = $promsel
"
...
@bind sprom NumberField(0.0:10.0:0.1; default=0.5)
...
md"- Cut on prominence ~ 0.5"

# ╔═╡ bc3ddf9f-76ca-4c78-b140-ed29747aaed2
md"- Cut on with ~ 6"
...
# ╔═╡ 3c0c557e-bbe3-49b7-b422-1217fb2f7fda
first(wvfm1)
...
wvfm3 = read_waveform(csvf,fileNumber);

# ╔═╡ 96dad766-398d-414c-ac66-dfaa76947ffc
md"
		sampling_rate(wvfm1, ws=5.0e-6)
  Given waveform **wvfm** with length **ws** (in seconds), returns the sampling rate
"

# ╔═╡ b43916ba-0499-4d82-b81c-49aba21ee145
sampling_rate(wvfm, ws=5.0e-6) = uconvert(GHz, (length(wvfm.Time) / ws) * Hz)


# ╔═╡ 8e99255c-0357-425f-978b-4bf3eb9428ce
begin
	fs = uconvert(Hz, sampling_rate(wvfm1))/Hz
	fl = 150.0MHz # filter high frequency noise
	flh = uconvert(Hz, fl)/Hz
end

# ╔═╡ 5dfbb58a-1411-4264-9540-8cc26263910f
md"
		sampling_period(wvfm, ws=5.0e-6)
  Given waveform **wvfm** with length **ws** (in seconds), returns the sampling period
"

# ╔═╡ ff872f5f-b10b-4065-8850-d81f9d6aece3
sampling_period(wvfm, ws=5.0e-6) = uconvert(ns, 1.0 / sampling_rate(wvfm, ws))

# ╔═╡ f25d1e02-4933-4864-9d34-bd061ced9eb8
md"- number of samples in the waveform (5 μs long) = $(length(wvfm1.Time))
- Sampling rate = $(round(sampling_rate(wvfm1)/GHz, sigdigits=2)) GHz
- sampling period = $(round(sampling_period(wvfm1)/ns, sigdigits=2)) ns"

# ╔═╡ cfaf2b26-e39d-47b5-8e99-8d35dbd7b3ac
md"
		fourier_transform(wvfm1)
  	
   Given waveform **wvfm1** returns the sampling period, fourier transform amplitude and frequency
"

# ╔═╡ 13e729c1-15ff-4fcf-b3df-9247fda54109
function fourier_transform(wvfm1)
	ts = sampling_period(wvfm1)
...
function plot_fft(wvfm1)
	ts, F, freqs = fourier_transform(wvfm1)
...
# ╔═╡ 10504712-31e8-4d00-8ebf-59d4feb0ff5f
plot_fft(wvfm1)
...
md"
	filter_signal(wvfm, flh, fs, filtertype, n=4, ϵ=1.0)

 Given waveform **wvfm**, with sampling rate **fs** (in seconds) and cutoff frequency **flh**, the function returns a filter (Butterworth, or Chebyshev), frequency response and filtered signal
"


# ╔═╡ 84c75a93-92ce-4df4-be33-9fd71bffb2ca
function filter_signal(wvfm, flh, fs, filtertype, n=4, ϵ=1.0)
...
# ╔═╡ 00c92e6b-749e-4858-96ee-be4a0a056fbb
hbw, fltbw, fsgnbw = filter_signal(wvfm1, flh, fs, "Butterworth",4, 1.0);

# ╔═╡ 27749777-9a67-42dc-91d4-e17ed99f8bb3
begin
	plot(wvfm1.Time, wvfm1.Ampl, lw=2, label="unfiltered")
	plot!(wvfm1.Time, fsgnbw, w=2, label="Butter filter")
	xlabel!("Time (μs)")
	ylabel!("Amplitude")
end

# ╔═╡ 78d6a1e6-f820-41ba-8050-ecaed514fd96
hcb, fltcb, fsgncb = filter_signal(wvfm1, flh, fs, "Chebyshev1",4, 2.0);

# ╔═╡ 88e18ad4-f478-4e5f-a239-91f18b781937
begin
	plot(wvfm1.Time, wvfm1.Ampl, lw=2, label="unfiltered")
	plot!(wvfm1.Time, fsgncb, lw=2, label="Chebyshev filter")
	xlabel!("Time (μs)")
	ylabel!("Amplitude")
end

# ╔═╡ 68df6f2c-f614-4f66-be31-180823f51edb
begin
	plot(abs.(hbw), lw=2, label="Butter")
	plot!(abs.(hcb), lw=2, label="Cheby")
	xlabel!("Samples")
	ylabel!("Frequency")
end

# ╔═╡ 76eed385-e4c0-4c25-9e8e-6d558b683f13
begin
	plot(amp2db.(abs.(hbw)), lw=2, label="Butter")
	plot!(amp2db.(abs.(hcb)), lw=2, label="Cheby")
	xlabel!("Samples")
	ylabel!("Amplitude")
end
...
function filter_signal_cb(wvfm1; fl=200.0MHz, n=4, ϵ=2)
	fs = uconvert(Hz, sampling_rate(wvfm1))/Hz
	flh = uconvert(Hz, fl)/Hz
	hcb, fltcb, fsgncb = filter_signal(wvfm1, flh, fs, "Chebyshev1",n, ϵ)
	wvfm1[!, "fAmpl"] = fsgncb
	wvfm1
end


# ╔═╡ 842d5c9d-598c-4565-8526-09b6df3a1d2f
fwvfm1 = filter_signal_cb(wvfm1; fl=200.0MHz);

# ╔═╡ 6da51a94-3d8b-447a-80fa-81eba8ced45c
first(fwvfm1,5)

# ╔═╡ d29de4fc-c875-4567-b9b8-4932a53c9d4e
fwvfm2 = filter_signal_cb(wvfm2; fl=200.0MHz);

# ╔═╡ b0e0c6da-7e0f-4923-a215-0559d0d072c6
fwvfm3 = filter_signal_cb(wvfm3; fl=200.0MHz);

# ╔═╡ f20b33b0-c0ec-4ee7-8273-6b738eb44dd7
function thrx(fwvfm1; nsigma=1, waveform="filtered")
	...
	meanfs = mean(fwvfm1[!, ampl])
	stdfs = std(fwvfm1[!, ampl])
	thrp = meanfs + stdfs * nsigma
	thrn = meanfs - stdfs * nsigma
	meanfs, stdfs, thrp, thrn
end

# ╔═╡ a461a88c-534b-465c-8dab-9d1e2dc49287
meanfs, stdfs, thrp, thrn = thrx(fwvfm1; nsigma=2.0)

# ╔═╡ 50304255-941d-4870-856d-c4a2de6f36a7
md" - After filtering: 
- mean = $(round(meanfs, sigdigits=2))   
- std = $(round(stdfs, sigdigits=2))
- thrp = $(round(thrp, sigdigits=2))   
- thrn = $(round(thrn, sigdigits=2)) 
"

# ╔═╡ 6a0f7c24-cc8f-4a14-8fdd-70ab6d9a9d06
wvfmFlt = filter(row -> row[:fAmpl] > thrp, fwvfm1);

# ╔═╡ f7bc85d0-c386-4c70-af24-9b2dd44544fb
upks, uvals = findmaxima(wvfmFlt.fAmpl)

# ╔═╡ 359ea578-cdcc-4a96-86e0-d334ad220702
pks, proms = peakproms(upks, wvfmFlt.fAmpl; minprom=promsel)

# ╔═╡ c09b06c6-f35b-4350-8012-5f907c53f8f3
_, uproms = peakproms(pks, wvfmFlt.fAmpl)
...
# ╔═╡ 1d36a8cc-b262-4b28-8ff6-47606f8b276d
_, widths, leftedge, rightedge = peakwidths(pks, wvfmFlt.fAmpl, proms)
...
pheigth = [wvfmFlt.fAmpl[i] for i in pks]

# ╔═╡ c5d56314-1690-4f04-92b0-00a8984b1e5c
ptime = [wvfmFlt.Time[i] for i in pks]

# ╔═╡ 13792c29-3966-49d2-a452-6fc8e9ef1f48
begin
	plot(wvfmFlt.fAmpl, lw=2, label=false)
	scatter!(pks, pheigth, label="peak")
	scatter!(leftedge, xbase, label="leftedge")
	scatter!(rightedge, xbase, label="rightedge")
...
meanAmp2, stdAmp2, thrp3s, thrn3s = thrx(wvfm2; nsigma=3.0, waveform="raw")

# ╔═╡ c31442a6-7480-422f-926d-f97c44996e22
fmeanAmp2, fstdAmp2, thrp2, thrn2 = thrx(wvfm2; nsigma=2.0, waveform="filtered")

# ╔═╡ 3979e3b9-dddc-433f-ab69-6fc1265bdf4f
wvfmFlt2 = filter(row -> row[:fAmpl] > thrp2, fwvfm2);

# ╔═╡ c0b2f470-5a64-41a6-a65b-e6c7c45bdc86
meanAmp3, stdAmp3, thr3p3s, thr3n3s = thrx(wvfm3; nsigma=3.0, waveform="raw")

# ╔═╡ 771ad8be-26d7-400f-b91b-850b4f5416be
fmeanAmp3, fstdAmp3, thrp3, thrn3 = thrx(wvfm3; nsigma=2.0, waveform="filtered")

# ╔═╡ d859f2d1-27bd-42c5-89a5-5ebffb9afc8b
wvfmFlt3 = filter(row -> row[:fAmpl] > thrp3, wvfm2);

# ╔═╡ 843d888f-1e8d-4875-b175-1bc0a98f3320
function plot_filtered_waveform(wvfm; window=false, waveform="filtered", wstart=-1, wend=-1, thrp=0, thrn=0)
...
# ╔═╡ 8e09dc6f-755e-49bb-8fde-ad29b9eb4bd2
plot_filtered_waveform(fwvfm1; window=false, wstart=701, wend=900, thrp=thrp, thrn=thrn)

# ╔═╡ 583a4fe4-07f7-4c1a-9420-bd2bcd0dcd93
plot_filtered_waveform(fwvfm1; window=true, wstart=501, wend=1500, thrp=thrp, thrn=thrn)

# ╔═╡ 53d6b897-7e60-4b24-ab6e-1e07be1ad43f
plot_filtered_waveform(fwvfm1; window=true, wstart=1200, wend=3500, thrp=thrp, thrn=thrn)

# ╔═╡ eac497f9-8c5a-42a9-af97-aaeddb3c0823
plot_filtered_waveform(wvfmFlt; window=false, thrp=thrp, thrn=thrn)

# ╔═╡ b3ddc6fe-6752-4f53-8eee-da30555fd95f
plot_filtered_waveform(wvfm2; window=false, waveform="raw", thrp=thrp3s, thrn=thrn3s)

# ╔═╡ 218b7cbc-641d-4b55-bcb3-f5572fffaa32
plot_filtered_waveform(fwvfm2; window=false, thrp=thrp2, thrn=thrn2)

# ╔═╡ a2f24a67-c112-4b10-94e8-10d078c29181
plot_filtered_waveform(wvfmFlt2; window=false, thrp=thrp2, thrn=thrn2)

# ╔═╡ 1cc2b8a5-87cd-4d84-a26d-905d5cec362e
plot_filtered_waveform(wvfm3; window=false, waveform="raw", thrp=thr3p3s, thrn=thr3n3s)

# ╔═╡ a09a6487-3bb6-4ece-ba5e-7b298a507666
plot_filtered_waveform(wvfm3; window=false, thrp=thrp3, thrn=thrn3)

# ╔═╡ 7b31594c-849e-4456-87fb-8224773f7a77
plot_filtered_waveform(wvfmFlt3; window=false, thrp=thrp3, thrn=thrn3)

# ╔═╡ 58d706c0-44f5-4514-b87b-eaf1f1940448
function select_filtered_peaks(wvfm, thrp; promsel = 0.1, wsel=2.5)
...
result = select_filtered_peaks(fwvfm2, thrp2; promsel = sprom, wsel=sw);
...
# ╔═╡ 83a0e714-a16f-4d73-8f86-64bb0bc8c54c
result3 = select_filtered_peaks(wvfm3, thrp3; promsel = sprom, wsel=sw);

# ╔═╡ 02203388-bfcc-4f9e-ad23-695155034be1
if result3 != Nothing
	wvflt3, speaks3 = result3
	println("")
end
...
# ╔═╡ bcb5df7a-66be-40a2-a17b-f1dd2ef70c64
plot_filtered_peaks(wvflt3, speaks3)

# ╔═╡ 2089aeed-5c35-4df0-bda6-f2162f3d3407
function prom_and_width(csvf, i0, il; fl=200.0MHz, nsigma=2.0, promsel=0.0, wsel=0.0)
	...
	for i in i0:il
		...
		fwvfm = filter_signal_cb(wvfm; fl=fl)
		fmeanAmp, fstdAmp, thrp, thrn = thrx(wvfm; nsigma=nsigma, waveform="filtered")
		result = select_filtered_peaks(wvfm, thrp; promsel = promsel, wsel=wsel)
...
PP, WP = prom_and_width(csvf, 1, 990)
...
# ╔═╡ 03492420-09ff-48fd-82d7-daadf2b41ec3
PP2, WP2 = prom_and_width(csvf, 1, 990; promsel=0.5)
...
hW2 = lfi.LaserLab.hist1d(WP2, 20, 0.0, 10.0)

# ╔═╡ d87eaa68-8c56-48c5-b19f-2f95c70f0e6b
lfi.LaserLab.hist1d(hW2, "Width (+)")

# ╔═╡ 24cf8503-fdde-4202-8f45-4b0a6b57108a
function select_peaks(csvf, i0, il; fl=200.0MHz, nsigma=2.0, promsel=0.5, wsel=6.0)
	SPK = []
	for i in i0:il
		wvfm = read_waveform(csvf,i)
		fwvfm = filter_signal_cb(wvfm; fl=fl)
		fmeanAmp, fstdAmp, thrp, thrn = thrx(wvfm; nsigma=nsigma)
		result = select_filtered_peaks(wvfm, thrp; promsel = promsel, wsel=wsel)
		if result != Nothing
			_, speaks = result
			push!(SPK,speaks)
...
# ╔═╡ 966845c7-03ed-4bed-bc9c-6c79af302b43
spks = select_peaks(csvf, 1, 990; fl=200.0MHz, nsigma=2.0, promsel=0.5, wsel=6.0)
...
fls650 = glob_files("../data/scope/signal_650nm/")

# ╔═╡ 938ff2ea-13af-4bce-98d9-8ef5349a7f3d
nfls650 = length(fls650)

# ╔═╡ 03b1941f-9dc7-4c5d-9188-ebd68e3d6cd7
spk650 = select_peaks(fls650, 1, nfls650; fl=200.0MHz, nsigma=2.0, promsel=0.5, wsel=6.0)
...
begin
	spk700nm = select_peaks(fls700, 1, nfls700; fl=200.0MHz, nsigma=2.0, promsel=0.5, wsel=6.0)
...
begin
	spk732nm = select_peaks(fls732, 1, nfls732; fl=200.0MHz, nsigma=2.0, promsel=0.5, wsel=6.0)
