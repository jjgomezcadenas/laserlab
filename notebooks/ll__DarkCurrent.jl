...
using Pkg; Pkg.activate("/Users/jjgomezcadenas/Projects/LaserLab/")
...
begin
	pmtdir  ="/Users/jjgomezcadenas/LaserLab/Proyectos/Data/PMT"
	odir  = "/Users/jjgomezcadenas/LaserLab/Proyectos/Data/PMT"
	dfiles ="*.trc"
	dplots ="*.png"
	md"""
	CMOS dir = $pmtdir
...
if zprom
	md""" Select min prominence cut: $(@bind spcutm NumberField(0.0:10.0, default=0.5))"""
end
...
md""" Select  prominence cut: $(@bind zpcut NumberField(0.0:10.0, default=0.2))"""
...
md"""
- First approach: simple fit of an exponential
"""

# ╔═╡ 9bf0b3e6-ef71-4eb6-9e7f-b27b46027e15
md""" Select initial bin for fit: $(@bind ibft NumberField(1:19, default=1))"""

# ╔═╡ 23c36d08-bb4e-4915-bcd7-c37d274f9d32
md""" Select final bin for fit: $(@bind ilft NumberField(1:19, default=15))"""

# ╔═╡ 93bf0910-9c92-4fb6-8969-decec34279e2
md"""
- Further possibilities: fit of 1 exp and 2 exp with background
"""
...
# ╔═╡ e74ec6e3-f2f8-4c10-b8df-1bb33a8520d2
rfpeaks
...
rate = 1e+3*length(fpeaks.xs) /200.0
...
if zprom
	...
	hproms  = lfi.LaserLab.hist1d(proms, 20, 0.0, 1.0)
	hwidths = lfi.LaserLab.hist1d(widths, 20, 0.0, 50.0)
...
#function subtract_laser(tv::Vector{Float64}, lt0::Float64,  ltus::Float64)
#	tv - ltus * floor.(tv/ltus)
#end

# ╔═╡ 8f177299-025f-4cfd-a5c7-130aec5f4de3
function subtract_laser(tv::Vector{Float64}, lt::Vector{Float64})
	#tv - ltus * floor.(tv/ltus)
	[closest_element(lt, x0) for x0 in tv]
end

# ╔═╡ d557f2b0-7ccf-45c7-a93b-5bf499cfafad
if rfpeaks != Nothing
	tsl = subtract_laser(fpeaks.xs, lpeaks.xs)
end
...
pmtts = filter(row -> row[:ts] > xtcut, pmtdf)

# ╔═╡ 5ad16ba5-92a6-4e2d-b044-e92825bd1446
begin
	xhpr = lfi.LaserLab.h1d(pmtts.pr, 20, 0.0, 1.0)
	...
	xhws = lfi.LaserLab.h1d(pmtts.ws, 20, 0.0, 50.0)
	xphws = lfi.LaserLab.plot_h1d(xhws, "width")
	xhvs = lfi.LaserLab.h1d(pmtts.vs, 20, 0.0, 2.0)
...
if zbkg
	coefx, stderx, tftx = tfit_bkg(htfit; pa0=[100.0, 0.5], i0=ibzb, l0=lbzb)
	md"""
	Background fit:
	- slope = $(round(coefx[2]))
	- cte   = $(round(coefx[1]))
...
# ╔═╡ a5dacbf3-5a7e-4c9a-985a-491d04f09ed3
coefx

# ╔═╡ 1146ca40-9d0d-4779-8d62-1ae4f991ada6
if zsgn && zbkg
	coefy, stdery, tfty = tfit_sgn(htfit; cbkg=coefx, pa0=[10000.0, 0.5], i0=ibzs, l0=lbzs)
...
if zbkg && zsgn && zsgn2
	...
	coefz, stderz, tftz = tfit_sgn2(htfit; cbkg=coefx, csgn=coefy, pa0=[1000.0, 0.5], i0=ibzs2, l0=lbzs2)
...
pmtt0 = filter(row -> row[:ts] < xtcut, pmtdf)

# ╔═╡ 1615fb7e-0f4c-4068-9a2e-dc4c62123342
length(pmtt0.ts) / length(pmtts.ts)

# ╔═╡ 9c35f578-100f-4ed8-933c-9b44dc902db4
...
function runsel(csvf::Vector{String}, fi::Integer, fe::Integer;
	...
	rate   = [length(spks[i].xs)/tw2  for i in 1:length(spks)]
	df, rate
end
...
begin
	hrate = lfi.LaserLab.h1d(xrate*1e+3, 30, 0.0, 30.0)
...
# ╔═╡ 3e22f705-802e-41c6-8a3b-59b8f23daa5e
begin
		coefxx, stderxx, tftxx =fit_peaks(htfit; pa0=[100.0, 0.5], i0=ibft, il=ilft)
		s1 = " cte = $(round(coefxx[1]))\n"
		s2 = " μ1 = $(round(coefxx[2]*1000)) ns\n"
		label = string(s1,s2)
		pngpath = joinpath(pngdir, string(sflt,"_simple_exp.png"))
	    ptime2 = plot_fit2(htfit, label, tftxx; savefig=true,
		                 fn=pngpath)
		plot(ptime2)
end
...
if zbkg
	let
		...
		pngpath = joinpath(pngdir, string(sflt,"_bkg.png"))
	   	ptime = plot_fit2(htfit, s1, tftx; savefig=true,fn=pngpath)
		plot(ptime)
	end
end
...
if zsgn && zbkg
	let
	    ...
	    ptime2 = plot_fit2(htfit, label, tfty; savefig=true,
		                 fn=pngpath)
		plot(ptime2)
	end
end
...
if zbkg && zsgn && zsgn2
	let
	    ...
	    ptime3 = plot_fit2(htfit, label, tftz; savefig=true,
		                 fn=pngpath)
		plot(ptime3)
	end
end
